...
let flag = true;
const banner = "--------->          TODO LIST (MEGA UPDATE)          <---------";
...
function colorred(index) {
    if (todo.length - index == 1) {
        ...
        let extract = todo.slice(index - 1, index);
...
while (flag) {
    ...
    if (selectedfunction.myfunction == chalk.redBright("View")) {
        ...
        if (todo.length == 0) {
            console.log(chalk.bgRedBright("Sorry but your todos is empty,please add your todos to get access to options"));
            flag = false;
        }
    ...
    if (selectedfunction.myfunction == chalk.yellow("Add")) {
        ...
        if (adding.pushcon == chalk.green("Add only one todo")) {
            ...
            const addcon = await inquirer.prompt({
                name: "add",
                type: "input",
                message: chalk.yellowBright("Enter your todo, please use || as seperator"),
            });
            ...
            if (more.settings == chalk.green("AddColors")) {
                while (flag5) {
                    ...
                    if (addcolor.color == chalk.magenta("Purple")) {
                        ...
                        if (typeof index.myindex == "number" &&
